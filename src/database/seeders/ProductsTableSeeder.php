...
class ProductsTableSeeder extends Seeder
{
    ...
    public function run()
    {
        $product_season = DB::table('seasons')->where('id', 3)->first();
        $param = [
            'name' => 'キウイ',
            'price' => 800,
            'image' => 'kiwi.png',
            'description' => 'キウイは甘みと酸味のバランスが絶妙なフルーツです。ビタミンCなどの栄養素も豊富のため、美肌効果や疲労回復効果も期待できます。もぎたてフルーツのスムージーをお召し上がりください!',
            'season' => $product_season->season_id
        ];
        $product_season = DB::table('seasons')->where('id', 1)->first();
        ...
        $product_season = DB::table('seasons')->where('id', 4)->first();
        DB::table('products')->insert($param);
        $param = [
            'name' => 'オレンジ',
            'price' => 850,
        ...
        $product_season = DB::table('seasons')->where('id', 2)->first();
        DB::table('products')->insert($param);
        $param = [
            'name' => 'スイカ',
            'price' => 700,
    ...
    {
        return $this->belongsToMany(Season::class)->withPivot('review');
    }
}
